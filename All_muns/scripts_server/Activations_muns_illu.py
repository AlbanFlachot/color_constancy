...
shape = (nb_mod, 330, 279)
conv1 = np.zeros((nb_mod, 330, 279, 16))
conv2 = np.zeros((nb_mod, 330, 279, 32))
conv3 = np.zeros((nb_mod, 330, 279, 64))
fc1 = np.zeros((nb_mod, 330, 279, 250))
fc2 = np.zeros((nb_mod, 330, 279, 250))
out = np.zeros((nb_mod, 330, 279, 1600))
predictions = np.zeros(shape)
EVAL = np.zeros(shape[:3])

dir_addr = '/home/alban/project_color_constancy/TRAINING/DATA/PNG/WCS/training_centered/'
for m in range(nb_mod):
	...
	for muns in range(330):
		img_addrs = glob.glob(dir_addr + '/object%i/*.npy'%muns)
		for ill in range(251):
			img_addr = img_addrs[ill]
			conv1[m,muns,ill], conv2[m,muns,ill], conv3[m,muns,ill], fc1[m,muns,ill], fc2[m,muns,ill], out[m,muns,ill],  predictions[m,muns,ill] = retrieve_activations(net, img_addr,'npy')
		EVAL[m,muns] = evaluation(predictions[m,muns],WCS_i[muns])
