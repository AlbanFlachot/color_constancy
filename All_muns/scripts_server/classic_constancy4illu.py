...
def computations_chromaticities(im_paths_mat, illus, shape, MUNSELLS_LMS, max_pxl, D65_correction = True , algo = 0):
    CHROMA_OBJ_MEAN, CHROMA_OBJ_LUM, CHROMA_OBJ_MEDIAN = comp.predicted_chromaticity(im_paths_mat,illus, shape, max_pxl, D65_correction)
...
DICT_ref = computations_chromaticities(im_paths_mat[:,3].reshape((nb_test_muns,1,nb_exp)), np.array([0.95595725, 1.00115551, 1.04288724]), (nb_test_muns, 1, nb_exp,1,3), MUNSELLS_LMS, max_pxl, D65_correction = True)
...
DICT_noCC = computations_chromaticities(im_paths_mat[:,[0,1,2,4]], np.array([1,1,1]), (nb_test_muns, 4, nb_exp, 1,3), MUNSELLS_LMS, max_pxl)
...
DICT_CCC = computations_chromaticities(im_paths_mat[:,[0,1,2,4]], mat_classic_cc_mat[:,[0,1,2,4]], (nb_test_muns,4,nb_exp,nb_algos,3), MUNSELLS_LMS, max_pxl)
